...
public:
	// These two constants are accessed by file-scope helper functions in
	// LoRaInterface.cpp (isSplitPacket / packetSequence), so they must be
	// public. The others (SEQ_UNSET, LORA_MAX_PAYLOAD) are only used by
	// member functions and stay private below.
